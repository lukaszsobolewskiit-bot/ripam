...
import { ArrowLeftRight, Plus, LayoutGrid, X, ChevronRight, Server, Cable, Info } from 'lucide-react'
...
function DeviceNode({ data }: { data: DeviceNodeData }) {
  ...
  return (
    ...
    >
      ...
      {!d.isExpanded && (
        <>
          <Handle type="target" id="h-left"   position={Position.Left}
            className="!w-2.5 !h-2.5 !border-2 !border-card" style={{ background: '#6b7280' }} />
          <Handle type="source" id="h-right"  position={Position.Right}
            className="!w-2.5 !h-2.5 !border-2 !border-card" style={{ background: '#6b7280' }} />
        </>
      )}
      ...
      <button
        className="flex items-center gap-3 px-4 py-3 rounded-t-xl w-full text-left bg-gradient-to-r from-primary/5 via-primary/3 to-transparent dark:from-primary/10 dark:via-primary/5 dark:to-transparent group"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            ...
            <ChevronRight className={cn('h-3.5 w-3.5 text-muted-foreground/60 transition-transform duration-300', d.isExpanded && 'rotate-90')} />
          </div>
          <div className="flex gap-3 text-[10px] text-muted-foreground mt-0.5">
            <span className="font-mono">{d.ip}</span>
            <span className="flex items-center gap-0.5"><Cable className="h-3 w-3" />{d.connectedCount}</span>
          </div>
        </div>
      </button>

      {/* Collapsed: model badge */}
      {!d.isExpanded && (d.deviceType || d.model) && (
        <div className="px-4 pb-2 flex items-center gap-1.5 text-[10px] text-muted-foreground">
          {d.deviceType && <span className="bg-muted rounded px-1.5 py-0.5 shrink-0">{d.deviceType}</span>}
          {d.model && <span className="truncate">{d.model}</span>}
        </div>
      )}

      {/* Expanded: port list with per-port handles */}
      {d.isExpanded && (
        <div className="px-2 pb-2 border-t border-border/30 pt-1.5 space-y-0.5">
          {d.ports.length === 0 && (
            <p className="text-[10px] text-muted-foreground italic px-2 py-1">No ports defined</p>
          )}
          {d.ports.map((port, idx) => {
            ...
            return (
              <div
                key={port.id}
                className={cn(
                  'relative flex items-center gap-2 rounded-md px-3 py-1.5 border',
                  port.connected
                    ? 'border-border/60 bg-muted/30'
                    : 'border-dashed border-border/40 bg-transparent hover:border-primary/40 hover:bg-primary/5',
                  'opacity-0 animate-[vlan-slide-in_0.2s_ease-out_forwards] transition-colors',
                )}
                style={{ animationDelay: `${idx * 15}ms` }}
              >
                {/* Source & target handles per port — these enable drag-to-connect */}
                <Handle
                  type="target"
                  position={Position.Left}
                  id={`pt-${port.id}`}
                  className="!w-3 !h-3 !border-2 !border-card !rounded-sm hover:!w-4 hover:!h-4 transition-all"
                  style={{ background: color, top: '50%' }}
                />
                <span className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: color }} />
                <span className="font-mono text-[11px] font-medium flex-1 truncate">{port.name}</span>
                <span className="text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0"
                  style={{ color, background: color + '22' }}>
                  {port.port_type.toUpperCase()}
                </span>
                {port.connected && <span className="w-1.5 h-1.5 rounded-full bg-green-400 shrink-0" />}
                <Handle
                  type="source"
          ...
          {/* Hint when expanded and has free ports */}
          {d.ports.some(p => !p.connected) && (
            <div className="flex items-center gap-1 pt-1 px-2 text-[9px] text-muted-foreground/60">
              <Info className="h-2.5 w-2.5 shrink-0" />
              Drag from a port handle to connect
            </div>
          )}
        </div>
      )}
...
// ─── Edge: floating bezier matching TopologyCanvas style ─────────────────────
...
function ConnectionEdge({ id, source, target, sourceHandle, targetHandle, data }: EdgeProps) {
  ...
  const { sx, sy, tx, ty, sp, tp } = useMemo(() => {
    ...
    const sr = {
      x: srcNode.internals.positionAbsolute.x, y: srcNode.internals.positionAbsolute.y,
      width: srcNode.measured.width ?? 240, height: srcNode.measured.height ?? 80,
    }
    const tr = {
      x: tgtNode.internals.positionAbsolute.x, y: tgtNode.internals.positionAbsolute.y,
      width: tgtNode.measured.width ?? 240, height: tgtNode.measured.height ?? 80,
    }
  ...
  const width = d?.isSelected ? 2.5 : 1.5
  ...
  return (
    <>
      ...
      <BaseEdge id={id} path={path}
        style={{ stroke: color, strokeWidth: width, strokeDasharray: '8 4', opacity: 0.9 }} />
      <BaseEdge id={`${id}-flow`} path={path}
        style={{ stroke: color, strokeWidth: width - 0.5, strokeDasharray: '4 8',
          opacity: 0.55, animation: 'dash-flow 2s linear infinite' }} />
      <EdgeLabelRenderer>
        {d?.portA && d?.portB && (
          <div className="absolute pointer-events-none"
            style={{ transform: `translate(-50%, -50%) translate(${lx}px, ${ly}px)` }}>
            <div className="rounded-md border bg-card/95 backdrop-blur-sm px-2 py-0.5 text-[9px] font-mono shadow-sm flex items-center gap-1 whitespace-nowrap"
...
// ─── Drag-connect confirmation dialog ────────────────────────────────────────

interface PendingConnection { portAId: number; portBId: number; portAName: string; portBName: string; hostAName: string; hostBName: string }

function DragConnectDialog({
  pending, onConfirm, onCancel,
}: { pending: PendingConnection; onConfirm: (desc: string) => void; onCancel: () => void }) {
  const [desc, setDesc] = useState('')
  return (
    <div className="space-y-4 p-1">
      <div className="rounded-lg border border-border bg-muted/30 p-3 text-xs space-y-1.5">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-foreground">{pending.hostAName}</span>
          <span className="text-muted-foreground font-mono">/ {pending.portAName}</span>
        </div>
        <div className="flex items-center gap-1 text-muted-foreground pl-1">
          <Cable className="h-3 w-3" />
          <span className="text-[10px]">connects to</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="font-semibold text-foreground">{pending.hostBName}</span>
          <span className="text-muted-foreground font-mono">/ {pending.portBName}</span>
        </div>
      </div>
      <div>
        ...
        <input
          ...
          placeholder="Cable label, patch info…"
...
function circleLayout(n: number) {
  ...
  if (n === 2) return [{ x: 100, y: 250 }, { x: 550, y: 250 }]
  const r = Math.max(260, n * 90)
...
function Inner({ projectId }: Props) {
  ...
  const createMutation = useMutation({
    mutationFn: (data: { port_a: number; port_b: number; description?: string }) =>
      portConnectionsApi.create({ ...data, project: projectId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['port-connections-all', projectId] })
      queryClient.invalidateQueries({ queryKey: ['hosts', { project: projectId }] })
      toast.success('Connection created')
      setPendingConn(null)
    },
    onError: (err: unknown) => {
      const msg = (err as { response?: { data?: { non_field_errors?: string[]; detail?: string } } })
  ...
  const visibleHosts = useMemo(
    () => (hosts ?? []).filter(h => connectedHostIds.has(h.id) || expandedNodes.has(String(h.id))),
    [hosts, connectedHostIds, expandedNodes]
  )

  const allHostsForExpand = hosts ?? []
  ...
  const toggleNode = useCallback((nodeId: string) => {
    setExpandedNodes(prev => {
      const n = new Set(prev); n.has(nodeId) ? n.delete(nodeId) : n.add(nodeId); return n
    })
  }, [])

  // Build port info lookup from hosts
  const portLookup = useMemo(() => {
    const m: Record<number, { name: string; hostId: number; hostName: string; portType: string }> = {}
    for (const h of (allHostsForExpand)) {
      const name = h.hostname || h.ip_address.split('/')[0]
      for (const p of (h.ports ?? [])) {
        m[p.id] = { name: p.name, hostId: h.id, hostName: name, portType: p.port_type }
      }
    }
    return m
  }, [allHostsForExpand])
  ...
  const onConnect = useCallback((connection: Connection) => {
    // Extract port IDs from handle IDs  (ps-{portId} or pt-{portId})
    const srcHandle = connection.sourceHandle ?? ''
    const tgtHandle = connection.targetHandle ?? ''
    const portAId = parseInt(srcHandle.replace(/^ps-/, ''))
    const portBId = parseInt(tgtHandle.replace(/^pt-/, ''))

    if (isNaN(portAId) || isNaN(portBId)) {
      toast.error('Please drag from a specific port (expand the device first)')
      return
    }
    if (portAId === portBId) {
      toast.error('Cannot connect a port to itself')
      return
    }

    const portA = portLookup[portAId]
    const portB = portLookup[portBId]
    if (!portA || !portB) return

    setPendingConn({
      portAId, portBId,
      portAName: portA.name, portBName: portB.name,
      hostAName: portA.hostName, hostBName: portB.hostName,
    })
  }, [portLookup])
  ...
  useEffect(() => {
    const displayHosts = visibleHosts.length ? visibleHosts : []
    if (!displayHosts.length && !(allHostsForExpand.length)) { setNodes([]); setEdges([]); return }

    const allVisible = allHostsForExpand.filter(h =>
      connectedHostIds.has(h.id) || expandedNodes.has(String(h.id))
    )
    if (allVisible.some(h => !posRef.current[String(h.id)])) {
      const layout = circleLayout(allVisible.length)
      allVisible.forEach((h, i) => {
    ...
    const newNodes = allVisible.map(h => {
      const nodeId = String(h.id)
      const isExpanded = expandedNodes.has(nodeId)
      const ports = (h.ports ?? []).map(p => ({
        id: p.id, name: p.name, port_type: p.port_type, connected: connPortIds.has(p.id),
      }))
      return {
    ...
    const newEdges = (connections ?? []).map(c => {
      ...
      return {
        ...
        type: 'connectionEdge',
        data: { portA: c.port_a_name, portB: c.port_b_name, color, isSelected: selId === String(c.id) },
      }
    })

    setNodes(newNodes as never)
    setEdges(newEdges as never)
  }, [allHostsForExpand, connections, selected, dtMap, expandedNodes, toggleNode, connectedHostIds])
  ...
  // Show all hosts (with ports) in the canvas for drag-to-connect
  const showAllBtn = allHostsForExpand.some(h => !connectedHostIds.has(h.id))
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">Add hosts with ports to this project first.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      >
        ...
        <Panel position="top-left">
          <div className="flex items-center gap-3 rounded-lg border border-border/50 bg-card/90 backdrop-blur-sm px-3 py-1.5 shadow-sm text-[11px] text-muted-foreground">
            <span className="flex items-center gap-1"><Server className="h-3 w-3" />{nodes.length}</span>
            <span className="flex items-center gap-1"><ArrowLeftRight className="h-3 w-3" />{connections?.length ?? 0}</span>
          </div>
        </Panel>
        ...
        <Panel position="top-right">
          <div className="flex flex-col gap-2">
            ...
            <button onClick={() => fitView({ padding: 0.25, duration: 400 })}
              className="flex items-center gap-1.5 rounded-lg border border-border/50 bg-card/90 backdrop-blur-sm px-2.5 py-1.5 text-xs font-medium text-foreground shadow-sm hover:bg-accent transition-colors">
            ...
            {showAllBtn && (
              <button
                onClick={() => {
                  const ids = new Set(allHostsForExpand.map(h => String(h.id)))
                  setExpandedNodes(ids)
                }}
                className="flex items-center gap-1.5 rounded-lg border border-border/50 bg-card/90 backdrop-blur-sm px-2.5 py-1.5 text-xs text-foreground shadow-sm hover:bg-accent transition-colors">
                <Server className="h-3.5 w-3.5" /> Show all devices
              </button>
            )}
        ...
        {selected && (
          <Panel position="bottom-center">
            <div className="flex items-center gap-2 rounded-lg border border-border bg-card/95 backdrop-blur-sm px-3 py-2 shadow-lg text-xs max-w-xl">
              <span className="w-3 h-3 rounded-full shrink-0 border-2 border-card shadow-sm"
                style={{ backgroundColor: portColor(selected.port_a_type) }} />
              <span className="font-mono font-semibold" style={{ color: portColor(selected.port_a_type) }}>{selected.host_a_name}</span>
              ...
              <span className="font-mono font-semibold" style={{ color: portColor(selected.port_b_type) }}>{selected.host_b_name}</span>
              <span className="font-mono text-muted-foreground text-[10px]">/{selected.port_b_name}</span>
              <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded uppercase shrink-0">{selected.port_a_type}</span>
              <div className="flex gap-1 ml-1">
                <button onClick={() => setAddOpen(true)}
                  className="rounded border border-border px-2 py-0.5 hover:bg-accent transition-colors">Edit</button>
                <button onClick={() => { if (window.confirm('Delete?')) deleteMutation.mutate(selected.id) }}
                  className="rounded border border-destructive/40 px-2 py-0.5 text-destructive hover:bg-destructive/10">Delete</button>
              </div>
      ...
      {/* Form dialog */}
      <Dialog open={addOpen} onOpenChange={(o) => { setAddOpen(o); if (!o) setSelected(null) }}
        title={selected ? 'Edit Connection' : 'Add Port Connection'}>
        <PortConnectionForm projectId={projectId} connection={selected ?? undefined}
          onClose={() => { setAddOpen(false); setSelected(null) }} />
      </Dialog>

      {/* Drag-to-connect confirmation */}
      <Dialog open={!!pendingConn} onOpenChange={(o) => { if (!o) setPendingConn(null) }}
        title="Confirm Port Connection">
        {pendingConn && (
          <DragConnectDialog
            pending={pendingConn}
            onConfirm={(desc) => createMutation.mutate({ port_a: pendingConn.portAId, port_b: pendingConn.portBId, description: desc })}
