...
class HostViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=["post"], url_path="copy-ports")
    def copy_ports(self, request, pk=None):
        """Manually trigger port copy from device model template."""
        host = self.get_object()
        if not host.device_model_id:
            return Response({"detail": "Host has no device model assigned."}, status=status.HTTP_400_BAD_REQUEST)
        existing = HostPort.objects.filter(host=host)
        if existing.exists():
            return Response({"detail": f"Host already has {existing.count()} port(s).", "count": existing.count()})
        templates = host.device_model.port_templates.all()
        if not templates.exists():
            return Response({"detail": "Device model has no port templates defined.", "count": 0})
        ports = HostPort.objects.bulk_create([
            HostPort(host=host, name=pt.name, port_type=pt.port_type, position=pt.position)
            for pt in templates
        ])
        return Response({"detail": f"Created {len(ports)} port(s).", "count": len(ports)})
...
class ManufacturerViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Manufacturer.objects.annotate(model_count=Count("device_models", distinct=True))
...
class DeviceModelViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = DeviceModel.objects.select_related("manufacturer").prefetch_related("port_templates")
...
class PortTemplateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = PortTemplate.objects.all()
        device_model = self.request.query_params.get("device_model")
        if device_model:
            qs = qs.filter(device_model_id=device_model)
        return qs
...
class HostPortViewSet(viewsets.ModelViewSet):
    serializer_class = HostPortSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = None

    def get_queryset(self):
        qs = HostPort.objects.all()
...
class PortConnectionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = PortConnection.objects.select_related(
            "port_a__host__subnet__project", "port_b__host", "project"
        )
        project = self.request.query_params.get("project")
        host = self.request.query_params.get("host")
        if project:
            qs = qs.filter(
                models.Q(project_id=project) |
                models.Q(project__isnull=True, port_a__host__subnet__project_id=project)
            )
        if host:
            qs = qs.filter(
    ...
    def perform_create(self, serializer):
        # Auto-assign project from port_a's host if not provided
        if not serializer.validated_data.get("project"):
            port_a = serializer.validated_data.get("port_a")
            if port_a:
                project = port_a.host.subnet.project
                serializer.save(project=project)
                return
        serializer.save()

class HostNoteViewSet(viewsets.ModelViewSet):
    serializer_class = HostNoteSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = None

    def get_queryset(self):
        qs = HostNote.objects.all()
        host = self.request.query_params.get("host")
        if host:
            qs = qs.filter(host_id=host)
        return qs
...
class HostFileViewSet(viewsets.ModelViewSet):
    serializer_class = HostFileSerializer
    permission_classes = [IsAuthenticated]
    pagination_class = None
    parser_classes = [parsers.MultiPartParser, parsers.FormParser]

    def get_queryset(self):
        qs = HostFile.objects.all()
        host = self.request.query_params.get("host")
        if host:
            qs = qs.filter(host_id=host)
        return qs
